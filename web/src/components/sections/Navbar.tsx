    ...
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-1">
          {[['#problem','Why TrustGrid'],['#features','Features'],['#who-we-serve','Who We Serve'],['#how-it-works','How It Works']].map(([href,label]) => (
            <a key={href} href={href} className="px-4 py-2 text-sm text-slate-500 hover:text-slate-900 transition-colors rounded-lg hover:bg-slate-50">{label}</a>
          ))}
        </div>

        <div className="hidden md:flex items-center gap-3">
          <a href="https://app.trustgrid.ng/login" className="text-sm text-slate-500 hover:text-slate-900 transition-colors font-medium">Sign In</a>
        ...
        <button onClick={() => setOpen(!open)} className="md:hidden text-slate-500 hover:text-slate-900">
      ...
      {open && (
        <div className="md:hidden bg-white border-t border-slate-200 px-6 py-4 space-y-1 shadow-lg">
          {[['#problem','Why TrustGrid'],['#features','Features'],['#who-we-serve','Who We Serve'],['#how-it-works','How It Works']].map(([href,label]) => (
            <a key={href} href={href} onClick={() => setOpen(false)} className="block py-3 text-sm text-slate-600 hover:text-slate-900 border-b border-slate-100 last:border-0">{label}</a>
          ))}
          <div className="pt-3 space-y-2">
            <a href="https://app.trustgrid.ng/login" className="block text-center py-2.5 text-sm border border-slate-200 rounded-xl text-slate-700 hover:bg-slate-50">Sign In</a>
            ...
            <a href="https://app.trustgrid.ng/worker-signup" onClick={() => setOpen(false)} className="block text-center py-2 text-xs text-slate-400 hover:text-slate-600">Joining as a worker? Sign up here →</a>
          </div>
        </div>
      )}
    </nav>
