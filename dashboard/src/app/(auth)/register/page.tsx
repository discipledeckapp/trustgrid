...
export default function RegisterPage() {
  ...
  const btnStyle = { background: `linear-gradient(135deg,${primaryColor},${accentColor})` }
  ...
  function goToStep2(e: React.FormEvent) {
    e.preventDefault()
    if (!institutionName.trim() || !institutionType || !institutionEmail.trim()) return
    setError('')
    setStep(2)
  }
  ...
  function goToStep3(e: React.FormEvent) {
    e.preventDefault()
    if (!firstName.trim() || !lastName.trim() || !phone.trim() || password.length < 8) return
    setError('')
    setStep(3)
  }
  ...
  // ── Shared page wrapper (NOT a component — just a function returning JSX) ──────
  const header = (
  ...
  // ── Step 3 — Review ───────────────────────────────────────────────────────────
  if (step === 3) return pageWrapper(
    <>
      <h1 className="text-xl font-black text-gray-900 mb-1">Review & create account</h1>
      <p className="text-gray-500 text-sm mb-5">Confirm your details before we set up your community.</p>
      ...
      <button onClick={() => setStep(2)} className="w-full text-gray-400 text-sm py-2 mt-2 hover:text-gray-600">
        ← Edit details
      </button>
    </>
  )
  ...
  if (step === 2) return pageWrapper(
    <>
      <h1 className="text-xl font-black text-gray-900 mb-1">Your admin account</h1>
      <p className="text-gray-500 text-sm mb-5">This will be the primary administrator for <strong>{institutionName}</strong>.</p>
        ...
        <div>
          ...
          <div className="relative">
            <input type={showPwd ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)}
              ...
              className="w-full border border-gray-200 rounded-xl px-4 py-3 pr-12 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500" required />
  ...
  return pageWrapper(
    <>
      <div className="flex items-center gap-3 mb-5">
        ...
        <div>
          <h1 className="text-xl font-black text-gray-900 leading-tight">Create your community</h1>
          <p className="text-gray-400 text-xs">Get started with TrustGrid in minutes</p>
        </div>
      </div>

      {/* Google sign-up */}
      <button type="button"
        onClick={() => alert('Google sign-in coming soon — complete the form below for now.')}
